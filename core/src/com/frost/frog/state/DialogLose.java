...
public class DialogLose extends ModuleDialog {
    private Button restart, menu;
    private BitmapFont bitmapFontW;
    private int score;

    public DialogLose(int x, int y, int w, int h, MyScreen p, int score) {
        super(x, y, w, h, p);
        restart = new Button(
                ...
                new Button.ButtonTask() {
                    @Override
                    public void task() {
                        ((Game) DialogLose.this.parent).restartGame();
                    }
                }
        ...
        generator.dispose();
        setInput(new DialogLose.Input(parent, restart, menu));
        this.score = score;
    }
